defmodule TeacherWeb.WidgetLive.OrderComponent do
  ...
  alias TeacherWeb.Components.Notifications.Alert
  ...
  def handle_event("buy", _params, socket) do
    ...
    case Purchaser.purchase(widget_id, quantity) do
      :ok ->
        opts = Flashy.Normal.Options.new(dismissible?: false)
        alert = Alert.new(:success, "You bought #{quantity} widget(s)!", opts)
        send_notification(socket, alert)
      :error ->
        alert = Alert.new(:warning, "Widget sold out")
        send_notification(socket, alert)
    end
